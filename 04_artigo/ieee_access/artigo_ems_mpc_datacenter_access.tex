...
\title{Predictive Energy Management of an Aggregated AI/HPC Data-Center Load with BESS: Comparative Evaluation under Detectable Events, and Embedded Controller Validation}
...
\begin{abstract}
This paper presents an applied, comparative energy management system (EMS) for an artificial intelligence / high-performance computing (AI/HPC) data center in which a battery energy storage system (BESS) provides uninterruptible support to an aggregated critical load. The contribution is not a new MPC theory: it is a reproducible protocol that separates forecast information from optimizer structure and evaluates five policies on one first-order plant---a reactive heuristic, a foresight-enabled heuristic (Heur+FS), a one-step myopic QP, a deterministic event-anticipating MPC-QP (golden V5, $N_p{=}60$, $N_c{=}15$), and a three-scenario stochastic MPC with a shared first action. Under nominal load steps, filtered unserved-power peaks of the deterministic MPC fall below \SI{1}{\kilo\watt} (raw $\approx\SI{0.018}{\kilo\watt}$), versus \SI{90.98}{\kilo\watt} (heuristic) and \SI{102.78}{\kilo\watt} (myopic). Under source loss at \SI{750}{\kilo\watt}, the same MPC reaches the case-study power floor $750{-}400=\SI{350}{\kilo\watt}$ ($\approx\SI{40.9}{\percent}$ peak cut, $\approx\SI{1.16}{\percent}$ energy cut); the floor is physics, not a solver failure. Timing error is more harmful than magnitude error. In the \emph{audited} protocol, a \SI{10}{\second} early forecast still sits on the case-study bound ($\approx\SI{350}{\kilo\watt}$); a \SI{20}{\second} late forecast raises the peak to $\approx\SI{512}{\kilo\watt}$ while remaining below the reactive $\approx\SI{593}{\kilo\watt}$. A historical $\approx\SI{699}{\kilo\watt}$ at $-10$\,s belongs to a previous campaign/protocol and is not reproduced here. A 200-pair Monte Carlo on the same physical trajectory shows that the stochastic competitor reduces the peak in 188/200 pairs and never worsens it beyond \SI{0.001}{\kilo\watt}, at a systematic SOC/throughput cost. An FPGA-ready SIL copy ($N_p{=}12$, $N_c{=}4$, ADMM, same UART protocol) reproduces the V5 floor to $\approx\SI{1}{\kilo\watt}$. UART hardware-in-the-loop on a Terasic DE2-115 closed the heuristic on four scenarios ($P_{\mathrm{un}}^{\max}$ identical to SIL). The on-chip ADMM runs, but Quartus Lite OpenCore Plus ($\approx\SI{1}{\hour}$) and soft-float QP time preclude a full 1200-step MPC serial campaign at $T_s=\SI{1}{\second}$. The plant remains aggregated: the board validates the embedded controller, not a MW facility and not UPS inner loops. Language throughout is load-continuity on this model, not a claim of UPS topology fidelity.
\end{abstract}

\begin{keywords}
Energy management system, model predictive control, BESS, data center, quadratic programming, hardware-in-the-loop, FPGA, comparative evaluation.
\end{keywords}
...
\item a five-controller protocol---heuristic, Heur+FS, myopic QP, deterministic MPC-QP (V5), stochastic three-scenario MPC---that isolates forecast versus multi-step optimization under identical plant dynamics;
\item evidence that, under nominal load-variation cases, deterministic MPC-QP drives filtered $P_{\mathrm{unserved}}$ below the reporting threshold, whereas heuristic and myopic QP remain limited by converter dynamics and rate limits;
\item forecast-error, multi-burst, public GPU/facility traces (NVML and DIPLOEE-derived), Monte Carlo, fairness/ablation and computational analyses, with a case-study power floor $P_{\mathrm{load}}-P_{\mathrm{BESS}}^{\max}$ stated as physics;
\item embedded validation of the FPGA-ready copy: SIL ($N_p{=}12$, $N_c{=}4$, ADMM) and UART HIL of the heuristic on a Terasic DE2-115, without claiming FIL, a completed MPC serial campaign, or $T_s=\SI{1}{\second}$ real-time ADMM on Cyclone~IV Lite.
\end{enumerate}

The central research question is whether the improvement comes merely from advance event information or from the combination of foresight with multi-step constrained optimization.
To answer this, the protocol compares a reactive heuristic, a foresight-enabled heuristic, a one-step myopic QP, and the event-anticipating MPC-QP under identical plant dynamics and constraints---so that each baseline isolates one factor (Table~\ref{tab:baseline_role}). The stochastic competitor additionally asks whether a cheap scenario fan-out reduces tail peaks at an SOC cost.
...
\midrule
Parisio \emph{et al.}~\cite{parisio2014mpc} & Microgrid operation & Mixed forecasts & Optimization-centric & Simulation & Economic dispatch, not AI steps \\
...
This paper & Aggregated source--load--BESS, $T_s=\SI{1}{\second}$ & Current measured; future forecast; unannounced $g$ hidden & Heur., Heur+FS, myopic, V5, 3-sc.\ stoch. & SIL + UART HIL (heuristic); ADMM on Nios shown & Timing/MC; case-study bound $750{-}400$ \\
\bottomrule
\end{tabular}}
\end{table*}
...
The fifth competitor is a scenario MPC built in this project: three internal trajectories (nominal forecast, future load ${+}20\%$, interruption announced \SI{10}{\second} early) with equal probabilities, a \emph{shared} command sequence, and scenario-specific predicted states and slacks. Only the first action is applied. Identical scenarios are merged by adding probabilities. This is not a claim of multi-stage recourse or of reproducing a named literature policy. Without an announced event, the early-interruption scenario coincides with the nominal one. Sensitivity with probabilities $0.50/0.25/0.25$ is reported as secondary.

The FPGA-ready copy used in Section~\ref{sec:embedded} is a different object: $N_p{=}12$, $N_c{=}4$, ADMM in C, same UART line protocol. It is not the V5 \texttt{quadprog} binary.
...
\label{sec:infopol}
Current measurements at $k$ are common to every controller. Future load, source limit and $g$ in the horizon are forecasts. Scheduled load steps, planned interruptions and protection alarms are distinct cues; an unannounced outage hides future $g$ from all five policies. Detection delay is a separate assay and is not a draw from the Monte Carlo delay law.

When foresight of $g$ is off, the assembler still fills future source-available / nominal limit into the QP: a causal return-to-normal hypothesis, not persistence of the measured outage. That hypothesis is declared, not claimed to be field-identified. A feedforward-plus-feedback competitor that would apply the same forecasts through a separate tracking loop was not added; the protocol already isolates forecast (Heur+FS), local QP (myopic) and multi-step QP (V5 and $N_c=N_p=1$). Repeating the same forecasts in a fourth structure would not resolve the unequal $\Delta u$ coefficients.
...
\begin{equation}
P_{\mathrm{BESS}}^{+}\approx b_b\cdot 400\approx\SI{157}{\kilo\watt}
\Rightarrow
P_{\mathrm{unserved}}\approx 750-157=\SI{593}{\kilo\watt}.
\end{equation}
...
\caption{Load bands (primary evidence): heuristic, myopic QP and MPC-QP---load, source, BESS, SOC and unserved power.}
...
\caption{Source limit: zoom on the $750\to 950$\,kW step. MPC-QP removes $P_{\mathrm{unserved}}$; myopic QP (102.78\,kW) exceeds the heuristic peak (90.98\,kW).}
...
\caption{Source loss: zoom of $P_{\mathrm{BESS}}$ and unserved power at fault onset; MPC-QP pre-positions to the $\approx 350$\,kW floor.}
\label{fig:loss_zoom}
\end{figure*}
...
\begin{tabular}{@{}lccc@{}}
\toprule
Mode & $P_{\mathrm{unserved}}^{\max}$ [kW] & Unserved energy [kWh] & Final SOC [\%] \\
\midrule
Foresight ON & 350.08 & 14.58 & 57.31 \\
Foresight OFF & 592.57 & 14.75 & 57.81 \\
\bottomrule
\end{tabular}
\end{table}
...
The converter time constant (Table~\ref{tab:tau}) keeps $P_{\mathrm{unserved}}^{\max}$ at the scenario-specific bound for $\tau_b\in\{1,2,4\}$\,s in this campaign ($350.02$--$350.08$\,kW). Residual fallback remains low (up to $\approx 2.6\,\%$ for $\tau_b=\SI{1}{\second}$ and $\SI{2}{\second}$; $<0.1\,\%$ for $\tau_b=\SI{4}{\second}$). Ranking among the five controllers is preserved under this first-order $\tau_b$ sweep; that does \emph{not} validate LCL/PLL/UPS dynamics.

\begin{table}[!t]
\centering
\caption{Sensitivity to $\tau_b$ (source loss, foresight ON)}
\label{tab:tau}
\begin{tabular}{@{}rcccc@{}}
\toprule
$\tau_b$ [s] & $P_{\mathrm{unserved}}^{\max}$ & Energy & FB & Mean $t_{\mathrm{QP}}$ \\
 & [kW] & [kWh] & [\%] & [s] \\
\midrule
1 & 350.03 & 14.58 & 2.58 & 0.045 \\
2 & 350.08 & 14.58 & 2.33 & 0.042 \\
4 & 350.02 & 14.58 & 0.08 & 0.044 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Prolonged critical outage}
Under an approximately \SI{2}{\hour} outage at constant \SI{750}{\kilo\watt} (outage window $[600,7800)$\,s, $T_f=\SI{9600}{\second}$), heuristic and MPC-QP were compared (Table~\ref{tab:outage}). Both reach $\mathrm{SOC}_{\min}=\SI{20}{\percent}$. After useful BESS depletion, $P_{\mathrm{unserved}}^{\max}=\SI{750}{\kilo\watt}$. Unserved energy is $1309.95$\,kWh (heuristic) vs.\ $1312.33$\,kWh (V5): MPC leaves $\approx 2.38$\,kWh \emph{more} unserved. Time with unmet power equals the $7200$\,s outage because the $400$\,kW rating is below the load. Final SOC $\approx 24.75\%$ / $24.73\%$ is \emph{after} restoration and recharge, not the SOC at $\mathrm{SOC}_{\min}$; pre-event SOC is $60\%$ / $59.50\%$. A $7200$\,s ``recovery'' column is the outage duration until the source returns, not autonomous BESS restoration. Closed-loop generator coordination is not simulated.
...
From the campaign CSV of the main comparison (MATLAB R2025b, Windows PC, Intel Core i7-4510U, \texttt{quadprog}/\texttt{interior-point-convex}), Table~\ref{tab:tqp} summarizes mean/max MPC-QP solve times and fallback by scenario. Wall-clock simulation time is larger because it includes plant updates and logging, not only the QP. The reported solve times therefore \emph{do not} demonstrate strict real-time operation at $T_s=\SI{1}{\second}$ on this conventional PC: occasional solves approach a non-negligible fraction of the sampling period, and deployment must budget computational headroom. Cycle time (assembly$+$QP$+$logging) must be reported separately from $t_{\mathrm{QP}}$. On the main V5 load-band job, cycle p50/p95/max $\approx 0.022/0.031/0.242$\,s with Hessian condition $\approx 3.3\times 10^{5}$. Deterministic V5 recorded no $T_s$ overruns in the principal short cases; the stochastic competitor recorded three first-sample overruns in its campaign and those are not hidden. The DE2-115 timings are in Section~\ref{sec:embedded}; they are not interchangeable with the i7 numbers. Warm-start speed-up is not claimed.

\begin{table}[!t]
\centering
\caption{MPC-QP solve times and fallback (main campaign)}
\label{tab:tqp}
\begin{tabular}{@{}lccc@{}}
\toprule
Scenario & Mean $t_{\mathrm{QP}}$ [s] & Max $t_{\mathrm{QP}}$ [s] & FB [\%] \\
\midrule
Load bands & 0.037 & 0.322 & 0 \\
Source limit & 0.046 & 0.131 & 0 \\
Source loss & 0.044 & 0.192 & 2.33 \\
Multi-burst & 0.041 & 0.228 & 0 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Forecast error, Monte Carlo, and limitations}
Under perfect foresight, MPC-QP reaches the case-study bound on source loss ($\approx\SI{350.07}{\kilo\watt}$). Magnitude errors of $\pm 10\%$ and $\pm 20\%$ in the predicted load (Table~\ref{tab:err_mag}) were \emph{tolerable}: the peak remains at that bound ($\approx\SI{350}{\kilo\watt}$). Timing shift of the predicted outage (Table~\ref{tab:err_time}) is the more sensitive axis, but the historical $\approx\SI{699}{\kilo\watt}$ at $-10$\,s is \emph{not} reproduced by the audited protocol. In the 11~September campaign the deterministic V5 peak at $-10$\,s is $350.00$\,kW---the same bound as the unshifted case---because the current sample remains measured while only the future forecast is shifted. None of the 41 grid points on $[-20,+20]$\,s makes V5 worse than the heuristic $592.61$\,kW. A $+10$\,s delay yields $355.57$\,kW; $+20$\,s yields $511.76$\,kW. For the descriptive criterion $P_{\mathrm{unserved}}^{\max}\le 351$\,kW, V5 passes on $[-20,+4]$\,s; that window is not a pre-registered requirement. An $N_{\mathrm{arm}}{=}8$ confirmation gate did \emph{not} improve the ${+}10$\,s Phase-1 re-run ($628$ vs.\ $363$\,kW ungated) and is not presented as a mitigation. The $699$\,kW figure belongs to a previous campaign/protocol; the difference is not attributed to the stochastic competitor, which the deterministic V5 does not use.
...
\midrule
0 & 350.08 & 14.58 & 2.33 \\
$-10$ & 350.13 & 14.86 & 2.00 \\
$+10$ & 350.17 & 14.58 & 2.83 \\
$-20$ & 350.07 & 15.18 & 1.00 \\
$+20$ & 350.09 & 14.58 & 2.58 \\
\bottomrule
\end{tabular}
\end{table}
...
Taken together, the campaign shows that the MPC-QP gain comes from the \emph{combination} of foresight and multi-step constrained optimization against converter dynamics---not from QP alone and not from foresight alone. The myopic QP does not pre-position the BESS: without a future step in the horizon it remains reactive under $\Delta u$ limits and ramp/SOC penalties, and can therefore perform \emph{worse} than the heuristic on the load step (102.78 vs.\ 90.98\,kW). Heur+FS captures part of the foresight benefit on isolated anticipable events, but does not resolve multi-event trade-offs well (multi-burst peak $60.65$\,kW vs.\ $0$\,kW for MPC-QP). The results support four messages:
\begin{enumerate}
\item under demand steps and source-limit saturation, MPC-QP drives filtered $P_{\mathrm{unserved}}$ below the reporting threshold in the nominal cases and outperforms heuristic and myopic QP;
\item the decisive mechanism is horizon-$N_p$ anticipation with multi-step optimization; Heur+FS is only a partial surrogate, and $N_{\mathrm{prep}}$ mainly shapes preparation;
\item under grid contingencies, MPC-QP mitigates the peak down to the scenario-specific bound $P_{\mathrm{load}}-P_{\mathrm{BESS}}^{\max}$ when foresight is adequate and well timed, but cannot overcome storage power/energy limits nor random outages without a cue;
...
Nomenclature is frozen as follows. \emph{SIL} is the FPGA-ready formulation ($N_p{=}12$, $N_c{=}4$) and the same ASCII UART protocol, with the plant in MATLAB (\texttt{backend=matlab\_fpga}). \emph{UART HIL} is the Nios firmware on a Terasic DE2-115 (Cyclone~IV~E EP4CE115F29C7, Quartus Prime Lite 20.1.1, 50\,MHz, 256\,kB on-chip), plant in MATLAB over RS-232 115200\,8N1. \emph{FIL} would be HDL Verifier over Ethernet; it was not implemented because the controller is C/ADMM on Nios, not an HDL DUT.

Table~\ref{tab:sil} compares heuristic and SIL-MPC against golden V5. SIL-MPC matches V5 on Group~A (0 vs.\ 0 filtered) and sits $\approx\SI{1}{\kilo\watt}$ above the 350\,kW floor on source loss ($351.17$ vs.\ $350.08$\,kW). Mean SIL $t_{\mathrm{QP}}$ is $7$--$11$\,ms; the first load-band solve reached $0.517$\,s on the PC (not the FPGA).

\begin{table}[!t]
\centering
\caption{SIL FPGA-ready vs.\ golden V5 ($P_{\mathrm{unserved}}^{\max}$ [kW])}
\label{tab:sil}
{\footnotesize
\begin{tabular}{@{}lccc@{}}
\toprule
...
\midrule
Load bands & 90.98 & 0.00 & 0.00 \\
Source limit & 90.98 & 0.00 & 0.00 \\
Source loss & 592.61 & 351.17 & 350.08 \\
Source return & 592.61 & 351.17 & 350.05 \\
\bottomrule
\end{tabular}}
\end{table}

UART HIL of the \emph{heuristic} on COM4 reproduced SIL to all reported digits on the four short scenarios ($P_{\mathrm{un}}^{\max}=90.98$ or $592.61$\,kW; wall $\approx 57$\,s; 0 fallback). That is closed-loop evidence for the protocol and the reactive law on the board. The ADMM firmware was linked into 256\,kB (228\,kB image, 26\,kB stack) by overlaying $H$/$K$ and dropping \texttt{printf}. Closed-loop ADMM UART executed (tens of \texttt{RSP} frames) until (i)~OpenCore Plus halted the time-limited Nios IP after $\approx\SI{1}{\hour}$, and (ii)~each QP exceeded $T_s$ by a large margin in software double on the Nios. A MATLAB row labelled \texttt{serial/mpc} with $P_{\mathrm{un}}^{\max}=90.98$ and $57$\,s wall was a switch-capture error (heuristic still selected) and is discarded. No FIL result is claimed. Cycle-time percentiles of ADMM \emph{on the FPGA} for 1200 steps are therefore not a paper table; the honest statement is that this Lite platform is adequate for the heuristic at $T_s=\SI{1}{\second}$ and for demonstrating ADMM execution, not for a 1\,s ADMM EMS.

\section{Data Availability}
\label{sec:data}

Campaign CSVs, job fingerprints, the FPGA-ready C sources, Qsys Tcl, the UART protocol, and the SIL/HIL logs used above are released as a tagged GitHub repository (\url{https://github.com/raonialderete-eng/ems-mpc-datacenter}, private until acceptance; tag \texttt{v1.0.0-ieee-access-rev}) and will receive a Zenodo DOI from that GitHub Release (placeholder \texttt{10.5281/zenodo.XXXXXXX} until the author publishes on Zenodo). The $\approx 1$\,GB raw DIPLOEE/NVML drop is not redistributed; the derived 1200\,s traces and \texttt{manifest.json} hashes are. MATLAB R2025b and Optimization Toolbox reproduce the desktop campaign; DE2-115 $+$ Quartus 20.1 are required only for UART HIL. SIL must not be cited as a MW installation test.

\section{Conclusion}
\label{sec:conclusion}
This paper presented an applied, comparative MPC-QP EMS for AI/HPC data-center energy management with BESS/UPS, together with a reproducible protocol that isolates forecast versus multi-step optimization and reports a fifth, three-scenario stochastic competitor. In nominal load-variation scenarios, deterministic MPC-QP drives filtered unserved-power peaks below \SI{1}{\kilo\watt}, while heuristic and myopic QP remain in the \SIrange{91}{109}{\kilo\watt} range; Heur+FS is intermediate on isolated steps and weaker on multi-burst. Under contingencies, MPC-QP reduces the initial transient to the case-study bound ($\approx\SI{350}{\kilo\watt}$ at \SI{750}{\kilo\watt} load) when foresight timing is adequate---about $40.9\%$ peak cut and $1.16\%$ energy cut versus the heuristic, not elimination of the outage deficit. Magnitude forecast errors within $\pm 20\%$ preserve that bound. In the audited timing grid a \SI{10}{\second} early shift remains on the bound; a \SI{20}{\second} late shift raises the peak to $\approx\SI{512}{\kilo\watt}$ while still beating the heuristic. Stochastic MPC compresses the Monte Carlo tail at a systematic SOC cost. SIL of the $N_p{=}12$ ADMM copy tracks V5; UART HIL on DE2-115 closes the heuristic. ADMM on Nios is demonstrated, not timed as a 1\,s product on Cyclone~IV Lite. Battery ageing and replacement cost are out of scope: $N_{\mathrm{EFC}}$ is a utilization proxy only.

The main message is fourfold. First, MPC improves predictive BESS coordination on an aggregated plant under AI-typical steps. Second, under source contingencies the benefit is bounded by installed power/energy and by forecast timing. Third, the method does not replace generators, a second feeder, or larger storage, nor does it validate UPS inner loops. Fourth, embedded evidence supports the controller-on-the-same-model claim, not a facility HIL and not FIL. Converter, SOC and efficiency parameters were not identified on a field battery; measurement noise is not injected. Code and CSVs will be archived as stated in Section~\ref{sec:data}.
...
$\Delta U=[\Delta u_1,\ldots,\Delta u_{N_c}]^\top$, $U=\mathbf{1}u_{k-1}+T\Delta U$ with $T_{ij}=\mathbb{1}(j\le i)$, and
\begin{equation}
P=\Phi p_k+\Gamma U=P^{0}+\Gamma T\Delta U,\quad
\Phi_i=a^{i},\quad
\Gamma_{ij}=ba^{i-j}\ \text{for }j\le i.
\end{equation}
...
&+w_s\|(S-\mathbf{1}s^\star)/100\|_2^2
+w_T((S_{N_p}-s^\star)/100)^2\\
&+w_p\|P/P_b\|_2^2+w_u\|\Delta U\|_2^2+\tfrac{\varepsilon}{2}\|z\|_2^2.
\end{aligned}
\end{equation}
...
p_{\min}\le P_i\le p_{\max},\quad
s_{\min}\le S_i\le s_{\max},\\
u_{\min,i}\le U_i\le p_{\max},\quad
u_{\min,i}&=\begin{cases}0,&\widehat G_i=0,\\p_{\min},&\widehat G_i=1,\end{cases}\\
-dU_{\max}&\le\Delta U\le dU_{\max},\\
P_{g,i}-\xi_{G,i}&\le\widehat M_i,\quad P_{g,i}\ge 0,
\end{align}
and $P_{g,i}=0$ if $\widehat G_i=0$. Recharge, when requested: $P_{g,i}+\xi_{R,i}\ge P_{g,i}^{\min,R}$. Event support, when positive: $P_i+\xi_{E,i}\ge q_i$. Otherwise the corresponding slacks are fixed at zero. There is no hard source-ramp inequality; ramp enters only the cost. Command rate is not the physical ramp of $p_k$. SOC inequalities use the tabulated bounds with zero extra margin.

\subsection{Event target and no-preview fill}
The raw target is $v_i=\min(\widehat L_i,400)$ during an outage and $v_i=\min(\max(\widehat L_i-\widehat M_i,0),400)$ with the source up. The first $v_i>0$ is $i_e$. If armed, $q_i=v_i$ for $i\ge i_e$; for $d=i_e-i\le 30$, $q_i=(30-d+1)v_{i_e}/30$; else $q_i=0$. If not armed, $q_i=0$. Source reference is zero on outage; $\max(0,\widehat L_i-q_i)$ when support is active; $\widehat M_i$ if load exceeds the limit without support; $\widehat L_i$ otherwise.

When foresight of $g$ is off, future $\widehat G$ and $\widehat M$ are filled as source-available / nominal limit (return-to-normal). That is causal and is not persistence of a measured outage.
...
\end{equation}
with a \emph{shared} $\Delta U$ and scenario-specific auxiliaries. There is no future branching. Dimension is $N_c+5N_p n_{\mathrm{unique}}\in\{315,615,915\}$. Only $u_k=u_{k-1}+\Delta u_1$ is applied.
...
IEEE Standards Association, ``Review of industry efforts and standards of grid readiness for data center deployment,'' IEEE SA Industry Connections, Piscataway, NJ, USA, 2025.
...
\bibitem{azizi2026gf}
A.~Azizi \emph{et al.}, ``Strengthening data center operations using grid-forming battery energy storage as a line-interactive uninterruptible power supply,'' \emph{Int. J. Electr. Power Energy Syst.}, vol.~170, Art.~no.~111638, 2026, doi: 10.1016/j.ijepes.2026.111638.
