...
Campaign CSVs, job fingerprints, the FPGA-ready C sources, Qsys Tcl, the UART protocol, and the SIL/HIL logs used above are available at \url{https://github.com/raonialderete-eng/ems-mpc-datacenter} and archived at \url{https://doi.org/10.5281/zenodo.22835283}. The $\approx 1$\,GB raw DIPLOEE/NVML drop is not redistributed; the derived 1200\,s traces and \texttt{manifest.json} hashes are. MATLAB R2025b and Optimization Toolbox reproduce the desktop campaign; DE2-115 $+$ Quartus 20.1 are required only for UART HIL. SIL must not be cited as a MW installation test.
