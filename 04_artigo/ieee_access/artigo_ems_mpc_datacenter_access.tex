...
\item a five-controller protocol on the 11~September campaign---heuristic, Heur+FS, myopic QP, deterministic MPC-QP (V5), stochastic three-scenario MPC---that compares different uses of forecast information and optimization under identical plant dynamics, plus a later FF+FB competitor (package B2) that uses the same forecasts;
...
Each controller answers a distinct design question (Table~\ref{tab:baseline_role}): the heuristic measures the performance of a simple reactive operating rule; Heur+FS measures a rule-based law that uses the same forecasts without a QP; the myopic QP measures constrained optimization without a future horizon; FF+FB (later package) measures a non-QP law with the same forecasts; MPC-QP measures the combined effect of foresight, a dynamic model and multi-step optimization. The 11~September protocol compares those controllers on the same plant; costs, $\Delta u$ coefficients and supervisory maps still differ, so the comparison is not a single-factor isolation. FF+FB is labelled separately.
...
To isolate the value of prediction \emph{within this rule-based law}, a foresight-enabled heuristic (Heur+FS) receives the \emph{same} future load, limit and $g$ window as the MPC-QP ($N_{\mathrm{prep}}$). Before a detected event it applies a proximity-proportional preparation ramp; at contingency onset it uses the same reactive rules. It does not solve a QP. Under the tested tuning that comparison favours V5; it does not by itself prove that every forecast-using non-QP law fails.
...
\item Fairness on the myopic $\Delta u$ limit, a 36-job myopic-weight cross (Section~\ref{sec:results}), and isolated ablations (foresight off, unit horizon, event inequality).
\item MATLAB SIL ($N_p{=}12$, \texttt{quadprog}) and UART HIL of the heuristic (Section~\ref{sec:embedded}).
\item CSV-checked later packages: repeated false alarms, horizon scale, additive measurement noise, and random detection delay (Section~\ref{sec:inclusion}).
\end{itemize}

\subsection{Inclusion of later packages}
\label{sec:inclusion}
Packages B2--B6 are not the 11~September five-column consolidation. A group enters a paper table or a paper claim only if (i)~$P/E/\mathrm{SOC}/\mathrm{FB}$ or cycle percentiles reconcile with the named job CSV/MAT, (ii)~the information fill is labelled, (iii)~the result changes or bounds a claim already in the protocol, and (iv)~preparation-energy, reversal-count and \texttt{aging\_proxy} are not used as KPIs. That gate admits FF+FB (B2) and the second-event limit test (B6\_segundo\_evento). The same check is reported for repeated false alarms (\texttt{B4\_alarmes}), myopic weights (\texttt{B3\_miope}), horizon scale (\texttt{B5\_horizonte}), measurement noise (\texttt{B6\_ruido}) and random detection delay (\texttt{B4\_deteccao}). Those five groups remain off Table~\ref{tab:punserved} for the technical reasons stated with each result (no physical outage; myopic-only persistence sweep; three horizon pairs on one PC; V5-only one-$\sigma$ noise; detection delay with foresight off). They are not omitted because the CSVs were left unread.
...
\end{equation}
Distance to that bound on source loss is $242.61$\,kW (heuristic), $12.47$\,kW (Heur+FS) and $\approx 0.07$\,kW (MPC-QP V5). Heur+FS is near the bound on a single contingency ($362.47$ vs.\ $350.07$\,kW) but, under the tested tuning, remains at $60.65$\,kW on multi-burst versus filtered $0$\,kW for V5. Package~B2 job \texttt{B2\_ff\_0005} (persistence) also reaches a filtered zero on that profile (raw $\approx 5.7\times 10^{-14}$\,kW vs.\ V5 \texttt{j00024} $0.018$\,kW), so the Heur+FS gap does not establish multi-step optimization as necessary. This bound is not a controller failure: it is $P_{\mathrm{load}}-P_{\mathrm{BESS}}^{\max}$ for this rating pair. Over $150$\,s at $350$\,kW the persistent energy floor is $14.5833$\,kWh. Even an optimal EMS cannot invent energy or power beyond the available storage.
...
Fairness: raising the myopic $\Delta u_{\max}$ from 120 to 200 to $400$\,kW/s on the source-limit case yields $102.78$, $71.31$ and $\approx 0$\,kW. Horizon is therefore not necessary for a near-zero peak in that isolated knob test. Package \texttt{B3\_miope} (36 jobs, persistence) crosses $w_{\Delta u}\in\{5,50,200,500\}$ with the same three $\Delta u_{\max}$ values. On load bands the peak is set by $\Delta u_{\max}$, not by $w_{\Delta u}$ ($102.78$ / $71.31$ / $\approx 0$\,kW at every weight). On source loss the myopic peak remains $702.78$ / $671.31$ / $592.61$\,kW by $\Delta u_{\max}$ and never reaches the $350$\,kW bound, so that ranking versus V5 is not an artefact of these weights. On NVML both knobs move the peak (from $28.3$\,kW at $w_{\Delta u}{=}5$, $\Delta u_{\max}{=}120$ down to $2.89$\,kW at $w_{\Delta u}{=}500$, $\Delta u_{\max}{=}400$). The myopic penalty is $w_{\Delta u}(u-u_{\mathrm{prev}})^2/P_{\mathrm{base}}^2$ whereas V5 uses $w_{\Delta u}\|\Delta U\|^2$ \emph{without} that $P_{\mathrm{base}}$ division ($P_{\mathrm{base}}=950$\,kW, $w_{\Delta u}=50$ $\Rightarrow$ coefficient ratio on the order of $9\times 10^{5}$ in normal mode). Equal weight \emph{names} do not equalize costs; the closest horizon ablation is V5 with $N_c=N_p=1$, not the separate myopic controller. With $\Delta u_{\max}=400$\,kW/s, multi-burst and NVML still favor MPC. Isolated removal of the event inequality changes source-loss V5 only from $350.067$ to $350.129$\,kW---keep the constraint, but do not call it the main gain. Without future preview, V5 source-loss returns to $592.56$\,kW. When foresight of $g$ is off, the audited assembler still writes future source-available / nominal limit into the QP (a causal ``return-to-normal'' hypothesis, not persistence of the measured outage).

Table~\ref{tab:ff} shows that FF+FB already drives the load-band filtered peak to zero (throughput $18.71$ vs.\ V5 $19.86$\,kWh; SOC$_f$ $59.11\%$ vs.\ $58.80\%$) and a source-loss peak of $355.95$\,kW versus V5 $350.07$\,kW (energy $14.5875$ vs.\ $14.5834$\,kWh). That source-loss margin is $\approx 5.88$\,kW of peak and only $\approx 4.15$\,Wh of energy, at $\approx 1.53$\,kWh extra V5 throughput and $0.42$\,pp lower SOC$_f$; it is not interchangeable with the $\approx 243$\,kW peak cut versus the reactive heuristic. On this plant, multi-step QP is therefore \emph{not} uniquely necessary for the Group~A zeros; V5 still sits closer to the $350$\,kW case-study bound on source loss.

\begin{table}[!t]
\centering
\caption{FF+FB (B2, persistence: \texttt{B2\_ff\_0001}/\texttt{B2\_ff\_0003}/\texttt{B2\_ff\_0005}) versus V5 (11~Sep: \texttt{j00004}/\texttt{j00014}/\texttt{j00024})}
\label{tab:ff}
...
Multi-burst & $\approx 0$ & 0.018 & 59.70 & 58.94 \\
\bottomrule
\end{tabular}}
\end{table}
...
\caption{Load bands, 11~September campaign, regenerated from jobs \texttt{j00001}/\texttt{j00003}/\texttt{j00004} (heuristic $90.98$\,kW / SOC$_f=59.51\%$; myopic $102.78$\,kW / $59.25\%$; V5 $0.018$\,kW / $58.80\%$, matching Table~\ref{tab:metrics}).}
...
\caption{Source limit, 11~September campaign, regenerated from jobs \texttt{j00006}/\texttt{j00008}/\texttt{j00009}: zoom on the $750\to 950$\,kW step. MPC-QP removes $P_{\mathrm{unserved}}$; myopic QP ($102.78$\,kW) exceeds the heuristic peak ($90.98$\,kW).}
...
\caption{Source loss, 11~September campaign, regenerated from jobs \texttt{j00011}/\texttt{j00013}/\texttt{j00014}: zoom of $P_{\mathrm{BESS}}$ and unserved power at fault onset; V5 peak $350.07$\,kW, SOC$_f=57.10\%$ (Table~\ref{tab:metrics}).}
\label{fig:loss_zoom}
\end{figure*}
...
Monte Carlo ($n{=}200$ paired forecast errors on one physical loss): V5 mean/p95/max peaks $359.05/384.82/580.32$\,kW; stochastic $350.01/350.03/350.67$\,kW. Stochastic reduces the peak in 188 pairs, ties 12, and never worsens it beyond \SI{0.001}{\kilo\watt}. Mean difference stochastic$-$V5 $= -9.04$\,kW (bootstrap 95\% CI $[-14.02,-4.79]$); median difference only $-0.16$\,kW---the gain sits in the tail. Mean final SOC $55.00\%$ (V5) vs.\ $48.67\%$ (stochastic) in all 200 pairs. Zero peaks worse than the heuristic in 200 draws is not risk zero: the two-sided 95\% binomial upper bound on that rate is $1.83\%$, conditional on the assumed error law. Drawn delays spanned about $-14$ to $+16$\,s, not the full $\pm 20$\,s clip. False alarm (stochastic): SOC $49.42\%$ vs.\ heuristic $60\%$, both without deficit. Package \texttt{B4\_alarmes} (24 jobs, four forecast-pulse patterns $\times$ six controllers, physical $g{=}1$) confirms that repeated false alarms keep $P_{\mathrm{un}}^{\max}\approx 0$: the heuristic does not move (SOC$_f=60\%$), while V5 spends SOC ($57.99$--$59.00\%$) and records $1.50$--$4.42\%$ fallback. Unannounced loss: V5 $\approx 592.56$\,kW. Detection delay is a distinct group: the 11~September 1/5/10\,s assays and package \texttt{B4\_deteccao} (40 V5 jobs, foresight off) give $749.91$\,kW on delayed trials and $592.56$\,kW at delay $0$. Additive measurement noise (\texttt{B6\_ruido}, 40 V5 jobs, $\sigma_L{=}5$\,kW, $\sigma_{\mathrm{SOC}}{=}0.2$\,pp) leaves source-loss peaks in $350.05$--$350.24$\,kW and NVML filtered peaks at 0. Do not confuse forecast error, detection delay, and sensor noise.
...
\item V5 outperforms Heur+FS and the myopic QP on the tested tunings; within-V5 ablations (foresight off, $N_c{=}N_p{=}1$) assess selected design choices and do not imply that multi-step QP is uniquely necessary;
...
The reproduction source for this revision is the GitHub repository \url{https://github.com/raonialderete-eng/ems-mpc-datacenter} (tag \texttt{v1.0.4-ieee-access-rev}) together with \texttt{07\_Experimental\_Pack.zip}: 11~September job MATs, \texttt{dataset\_v1} traces required by \texttt{caso\_auditado.m}, B2--B6 \texttt{jobs.json} and summary CSVs including \texttt{B2\_ff\_0005}, and the audited MATLAB under \texttt{01\_matlab\_base/revisao\_operacional}. That ZIP is attached to the GitHub release of the same tag and is supplied to the editor. The matching Zenodo version is \url{https://doi.org/10.5281/zenodo.22846250}. The earlier record \url{https://doi.org/10.5281/zenodo.22837544} is a snapshot of tag \texttt{v1.0.2-ieee-access-rev} and must not be used to reproduce the tables or figures of this revision. Synthetic cases are generated by \texttt{gera\_cenario}; NVML/DIPLOEE cases load \texttt{dataset\_v1}. Figures~2--4 were regenerated from jobs \texttt{j00001}/\texttt{j00003}/\texttt{j00004}, \texttt{j00006}/\texttt{j00008}/\texttt{j00009} and \texttt{j00011}/\texttt{j00013}/\texttt{j00014} and match the tables (source-loss V5 SOC$_f=57.10\%$, $350.07$\,kW). A MATLAB row labelled \texttt{serial/mpc} at $90.98$\,kW / $57$\,s wall was a switch-capture error and is not in the HIL CSV. The $\approx 1$\,GB raw DIPLOEE/NVML drop is not redistributed; reconstruction of mapped windows from that drop is an external dependency. MATLAB R2025b and Optimization Toolbox reproduce the desktop campaign; DE2-115 $+$ Quartus 20.1 are required only for UART HIL. SIL must not be cited as a MW installation test, as FPGA ADMM, or as UART-protocol validation.

\section{Conclusion}
\label{sec:conclusion}
This paper presented an applied, comparative EMS for AI/HPC data-center energy management with BESS/UPS, together with a reproducible protocol that compares different uses of forecast information and optimization, complemented by within-V5 ablations. The 11~September campaign reports heuristic, Heur+FS, myopic QP, V5 and a three-scenario stochastic competitor that shares the whole $\Delta U$; a later FF+FB package is labelled separately. In nominal load-variation scenarios, both V5 and FF+FB drive filtered unserved-power peaks below \SI{1}{\kilo\watt}, while heuristic and myopic QP remain in the \SIrange{91}{103}{\kilo\watt} range on load bands and source limit (myopic multi-burst $108.90$\,kW); Heur+FS is intermediate on isolated steps and weaker on multi-burst, whereas FF+FB also reaches a filtered zero on that profile (\texttt{B2\_ff\_0005}). Under contingencies, V5 reduces the initial transient to the case-study bound ($\approx\SI{350}{\kilo\watt}$ at \SI{750}{\kilo\watt} load) when foresight timing is adequate---about $40.9\%$ peak cut and $1.16\%$ energy cut versus the heuristic, not elimination of the outage deficit. FF+FB reaches $355.95$\,kW on the same source-loss case. Magnitude forecast errors within $\pm 20\%$ preserve that bound. In the audited timing grid a \SI{10}{\second} early shift remains on the bound; a \SI{20}{\second} late shift raises the peak to $\approx\SI{512}{\kilo\watt}$ while still beating the heuristic. Stochastic MPC compresses the Monte Carlo tail at a systematic SOC cost, and fails a depleted second-event limit test ($750$\,kW). SIL using the MATLAB FPGA backend is MATLAB/\texttt{quadprog} at $N_p{=}12$, not ADMM~C and not UART. UART HIL on DE2-115 closed the heuristic on four short runs without a recorded timeout; firmware $t_{\mathrm{us}}=0$, so cycle WCET is not measured. ADMM on Nios is demonstrated, not timed as a 1\,s product on Cyclone~IV Lite. Battery ageing and replacement cost are out of scope of this load-continuity comparison: $N_{\mathrm{EFC}}$ tracks throughput and is not a lifetime or replacement-cost model; the later \texttt{aging\_proxy} field is a multiple of the same throughput and is not used as a KPI.

The main message is fourfold. First, predictive BESS coordination on this aggregated plant removes converter-limited Group~A transients; MPC-QP is sufficient for that, and FF+FB shows it is not uniquely necessary. Second, under source contingencies the benefit is bounded by installed power/energy and by forecast timing. Third, the method does not replace generators, a second feeder, or larger storage, nor does it validate UPS inner loops. Fourth, embedded evidence supports the controller-on-the-same-model claim, not a facility HIL, not FIL, not UART-protocol SIL, and not FPGA ADMM SIL. Converter, SOC and efficiency parameters were not identified on a field battery. Additive measurement noise at $\sigma_L{=}5$\,kW and $\sigma_{\mathrm{SOC}}{=}0.2$\,pp (40 V5 jobs) does not reverse the source-loss bound or the NVML zeros; that check is not a converter/sensor identification. Code, job MATs and CSVs are archived as stated in Section~\ref{sec:data}.
