...
The reproduction source for this revision is the GitHub repository \url{https://github.com/raonialderete-eng/ems-mpc-datacenter} (tag \texttt{v1.0.4-ieee-access-rev}) together with the confidential experimental pack supplied to the editor: 11~September job MATs, \texttt{dataset\_v1} traces required by \texttt{caso\_auditado.m}, B2--B6 \texttt{jobs.json} and summary CSVs, and the audited MATLAB under \texttt{01\_matlab\_base/revisao\_operacional}. Synthetic cases are generated by \texttt{gera\_cenario}; NVML/DIPLOEE cases load \texttt{dataset\_v1}. Figures~2--4 were regenerated from jobs \texttt{j00001}/\texttt{j00003}/\texttt{j00004}, \texttt{j00006}/\texttt{j00008}/\texttt{j00009} and \texttt{j00011}/\texttt{j00013}/\texttt{j00014} and match the tables (source-loss V5 SOC$_f=57.10\%$, $350.07$\,kW). The Zenodo record \url{https://doi.org/10.5281/zenodo.22837544} is a snapshot of tag \texttt{v1.0.2-ieee-access-rev}; it does not match this manuscript and must not be used to reproduce the tables or figures of this revision. The reviewer-facing archive of this revision is the confidential experimental pack together with the GitHub tag. A MATLAB row labelled \texttt{serial/mpc} at $90.98$\,kW / $57$\,s wall was a switch-capture error and is not in the HIL CSV. The $\approx 1$\,GB raw DIPLOEE/NVML drop is not redistributed; reconstruction of mapped windows from that drop is an external dependency. MATLAB R2025b and Optimization Toolbox reproduce the desktop campaign; DE2-115 $+$ Quartus 20.1 are required only for UART HIL. SIL must not be cited as a MW installation test, as FPGA ADMM, or as UART-protocol validation.
