...
% Fill after Zenodo Publish; GitHub URL is set after `gh repo create`.
\newcommand{\repgithub}{https://github.com/raonialderete-eng/ems-mpc-datacenter}
\newcommand{\repzenodo}{10.5281/zenodo.XXXXXXX}
...
\textit{Code:} \url{\repgithub} (private until acceptance), tag \texttt{v1.0.0-ieee-access-rev}. Zenodo DOI \texttt{\repzenodo} after the author publishes the GitHub Release (see \texttt{ZENODO\_DOI\_PASSO.md}).
...
\textbf{(b) Response.} GitHub (versioned, private until acceptance): \url{\repgithub}, tag \texttt{v1.0.0-ieee-access-rev}. Zenodo DOI \texttt{\repzenodo} is minted by the author from that Release (not from this engineering session). Raw $\approx 1$\,GB traces are not uploaded; \texttt{manifest.json} hashes are.
