...
% - mpc_qp_v5
%
% Esta versão foi revisada para evitar:
% - switch sem end;
% - for interno sem fechamento correto;
% - cálculo de métricas dentro do loop k;
% - tabela sendo montada antes de fechar o loop dos cenários;
% - repetição indevida dos mesmos cenários.
...
% Cenários curtos para validação completa da V5
cenarios = { ...
    'carga_faixas', ...
    'limite_fonte', ...
    'perda_fonte', ...
...
% Para rodar somente um cenário:
% cenarios = {'perda_fonte'};
...
switch tipo_controle
    ...
    case 'mpc_qp_v5'
        nomes_controle = 'MPC-QP-V5';

    otherwise
        error('Tipo de controle não reconhecido. Use heuristico, mpc_qp_v3, mpc_qp_v4 ou mpc_qp_v5.');

end

%% 4. Configurações gerais

gerar_graficos = false;   % true gera PNGs; false acelera e evita travar em -batch
...
for i = 1:n_cenarios
    ...
    for k = 1:N-1
        ...
        switch tipo_controle
            ...
            otherwise
                ...
                error('Tipo de controle não reconhecido durante a simulação.');

        end  % fim do switch tipo_controle
    ...
    if strcmp(tipo_controle, 'mpc_qp_v3') || ...
       strcmp(tipo_controle, 'mpc_qp_v4') || ...
       strcmp(tipo_controle, 'mpc_qp_v5')
    ...
    resultados(i).exitflags_mpc = exitflags_mpc;
    ...
    if gerar_graficos == true
        ...
        figure('Name', [nome_cenario_char ' - ' nomes_controle], 'NumberTitle', 'off');

        subplot(5,1,1)
        plot(tempo_plot, Pload, 'LineWidth', 1.8)
        grid on
        ylabel('Pload [kW]')
        title([nome_cenario_char ' - ' nomes_controle])
...
end  % fim do loop principal dos cenários
